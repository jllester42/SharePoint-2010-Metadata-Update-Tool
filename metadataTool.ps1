...
$fileFolder = "C:\test\"
# REMOVE: 2nd file folder for testing
#$fileFolder = "C:\Users\jonal90\Desktop\TestFiles"
...
# The value below can be removed, following with a modification to buildTable() function, once localized testing has been completed.
# builtTable will then use csvFile variable representing the created file that is saved to the server, 
$serverCSVFile = "'C:\jonal90\outputFile2.0-NE.csv'"
$serverCSVFile = "'C:\jonal90\outputFile3.0-NE.csv'"

# The location/name of the CSV file, that is used to store extracted metadata information.
$CSVFile = "C:\scripts\outputFile.csv"

# This is the temporary renaming of the blob database being used for testing, this value can be deleted once testing has been completed.
$blobDatabase = "TestBlob"
...
function createFilesFromBLOB()
{
	...
	# Creates a format file to be used to recreate the file
	$formatQuery = "BCP $blobDatabase format nul -n -f '$formatFile' -T"
	
	$formatCmd = New-Object System.Data.SqlClient.SqlCommand
	$formatCmd.CommandText = $formatQuery
	$formatCmd.Connection = $SqlConnection
	...
	for ($count = 0; $count -lt $totalCount; $count++) {	
		...
		# Modify $blobQuery to reflect actual database names/values
		$blobQuery = "BCP 'SELECT Content FROM $SPSqlTable where ID = $idArray[$count]' QUERYOUT '$saveTo' -n -T -f '$formatFile' -S"
					
		$blobCmd2 = New-Object System.Data.SqlClient.SqlCommand
		$blobCmd2.CommandText = $blobQuery
		$blobCmd2.Connection = $SqlConnection
...
function extractMetadata($location)
{
	...
	ForEach($sFolder in $location ) {
		...
		ForEach ($file in $objFolder.Items()) {
			...
			{
				#Creates a hash table to temporarily store the metadata values.
				$hash += @{ `
			...
			# Creates an xml file using the values from the hash table generated earlier.
...
function readInDatabaseValues{
	...
	if ($rowNumber -eq $SProwNumber)
	{
		for ($index = 0; $index -lt $SProwNumber; $index++)
		{
			...
			# Replace with commented out code below that corresponds to sharepoint database layout
			if ( !(IsNullOrEmpty "$($SPDataSet.Tables[0].Rows[$i][1])"))
			{
				$SPFilename = "$($SPDataSet.Tables[0].Rows[$i][1])"
	
				$SPDateLastSaved = "$($SPDataSet.Tables[0].Rows[$i][0])"
	
				$SPContentCreated = "$($SPDataSet.Tables[0].Rows[$i][2])"
			}
				
			# Replace code above with this infomration
